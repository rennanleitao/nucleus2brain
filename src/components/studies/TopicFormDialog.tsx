...
export function TopicFormDialog({ open, onOpenChange, topic, defaultAreaId }: Props) {
  ...
  const [status, setStatus] = useState<StudyTopicStatus>("monitorar");
  ...
  useEffect(() => {
    ...
    setCurrentReading(topic?.current_reading ?? "");
    setStatus((topic?.status as StudyTopicStatus) ?? "monitorar");
    ...
    try {
      if (topic) {
        await update.mutateAsync({
          ...
          current_reading: currentReading.trim() || null,
          status,
          tags: tagArr,
        });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              ...
              <Select value={status} onValueChange={(v) => setStatus(v as StudyTopicStatus)}>
                ...
                <SelectContent>
                  {(Object.keys(TOPIC_STATUS_LABELS) as StudyTopicStatus[]).map((s) => (
                    <SelectItem key={s} value={s}>{TOPIC_STATUS_LABELS[s]}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
          ...
          <div className="space-y-1.5">
            <Label>Leitura atual (opcional)</Label>
            <Textarea
              value={currentReading}
              onChange={(e) => setCurrentReading(e.target.value)}
              rows={4}
              placeholder="Sua interpretação atual sobre o tema"
            />
          </div>
          <div className="space-y-1.5">
